...
**预期测试数逐任务：** 631 → T1 633 → T2 637 → T3 644 → T4 不变 → T5 645（复审后补的 `pctText` 一条）→ T6–T7 不变 → 整支审查后的修复 647（`TAG_TEXT` 两条）。终态 **46 文件 / 647 通过 / 3 跳过**。
...
- **degraded 决策点：不要靠碰运气，它碰不到。** 控制方已实测：`playAiHand` + `analyzeHand` 跑 **180 手 / 278 个决策点（其中 35 个是 3 人以上局面）→ 零个 degraded**（400 迭代与生产 1500 迭代两档都跑过）。这与代码库自己早先的结论一致 —— `src/review/analyzeHand.degraded.test.ts` 的文件头注释记载：扫过 an-1..an-9 等全部种子后确认真实对局里 `if (d.degraded)` 分支永不成立，那条断言「断言了但什么都没测到」，所以那份测试改用 `vi.mock` 强制。

  原因：降级要求多个对手的继续范围**互相冲突到采不出样**（`evEstimate.ts:340-362` 的 `InfeasibleSamplingError`），而不只是人多；性格驱动的范围足够宽，几乎不会塌缩到同一窄类别。`evEstimate.test.ts:563` 里是把三个对手的范围全部写死成 `AA` 才逼出来的。

  所以验收改为**临时强制**：把 `src/core/evEstimate.ts:210` 的
  `degraded: degradeTracker.maxWidened > 0 ? 'widened-ranges' : null,`
  临时改成 `degraded: 'widened-ranges',`，在浏览器里打一手、打开复盘，确认：
  - 每一行都显示「无法判定」，色点是 unknown 档
  - 展开后**没有任何 EV 数字、条形图、推荐动作或 tag**，只有底池 / 待跟注 / 所需胜率 / 那句降级文案
  - 整体评级显示为 unknown 档而不是「没问题」
  验完立刻 `git checkout src/core/evEstimate.ts` 还原，并**跑一次 `npm.cmd test` 确认回到 645**。报告里必须写明这一条是强制注入验的、不是自然出现的手牌。
