...
架构守卫 2 条）→ T3 695（统计 23 条）→ T4 714（仓库 19 条）→ T5 736（筛选 22 条）
→ T6/T7 **758**（导出导入 17 条 + 仓库导入 5 条）。终态 **54 文件 / 755 通过 / 3 跳过**。
...
**导出不含统计。** 统计是从手牌推出来的派生值。一起写进文件就多了一份可能
与手牌对不上的真相——用户手改文件、或把两份导出合并时，先坏的一定是它。
导入侧统一重算，代价是一次全表扫描，换来「库里的统计永远等于手牌算出来的
统计」这条不变量。浏览器验收时故意把 stats 写成 999，导入后自动回到 7。

**导入必须整表重算，不能接着增量。** `applyHand` 的 `lastHandId` 只挡得住
「同一手连着来两次」，而导入是**乱序插入**——一次塞进来几百手，时间戳与库里
已有的交错。这正是 T3 那条「已知边界」测试预告的情形。重算时按 timestamp
排序，否则滚动窗口 `recentNet` 会变成「getAll 恰好返回的那些手」。

**申请持久化存储。** 浏览器验收中途发现 `navigator.storage.persisted()` 是
false——默认的 IndexedDB 是 best-effort，浏览器可以在磁盘紧张时把整份数据
丢掉且不通知任何人。对一个专门存历史的应用这不可接受，启动时调
`storage.persist()` 申请；拿不到也不影响启动。
