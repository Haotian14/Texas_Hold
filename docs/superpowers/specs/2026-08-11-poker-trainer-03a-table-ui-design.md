...
13. 赢到超过 6000（150BB）时深筹码标记出现。标记**不会**因为 hero 回落而消失——`isDeepStackHand`（`handSession.ts`）判定的是「本手开局时」全部座位（含 AI）里是否有任一座位 ≥ 150BB，只要还有其他座位开局达标，hero 自己缩水到远低于 150BB 时标记仍然显示（§4.6 就是这样定义的，不是 bug）
