...
export function rankRange(
  ...
  // 胜率略微升高，查表拿不到这个量。这是近似，不是恒等式——换牌效应并不是
  // 同向的：剔除 hero 自己的 AhAd 会压低每一个含 A 的类别（相对 76s 这类不含
  // A 的类别）的胜率，这正是「阻断牌」本来的定义，排序确实可能因此发生局部
  // 扰动，不是"大家一起同向偏移、名次不变"。但这个效应的量级小于表本身的
  // 采样噪声：PREFLOP_TABLE_SAMPLES = 1500 次蒙特卡洛，二项分布标准误差
  // ≈ sqrt(0.5×0.5/1500) ≈ 1.3 个百分点（p 取最坏情形 0.5），换牌效应本身
  // 通常只有零点几个百分点，量级上不足以在实践中稳定改变依赖这个排序的
  // 下游决策，这一点本身足以撑住不做换牌修正这个近似，不需要再借助
  // "排序不受影响"这个不成立的说法。
