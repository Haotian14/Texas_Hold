...
const SELECTORS = [
  ...
  // 动作条内部。加价档与金额框在窄屏上会互相顶，金额框曾被顶出面板 30px
  // 再被视口切掉（屏幕上是「$120 | 最」）。
  // `.raise-step` 是**专门为这个清单加的类名**（见 ActionBar.tsx）：那几颗
  // 按钮的样式全是工具类。这里原来写的是 `.preset`，而界面上从来没有过那个
  // 类名——一条永远选不中任何元素的规则，空跑了整整一轮改版还看不出来。
  // 往这个清单里加选择器时先确认它真的能选中东西。
  '.raise-step', '.raise-amount-box',
];
